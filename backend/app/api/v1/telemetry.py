...
from app.services.ingestion import resolve_headers, clean_and_validate_row
...
async def upload_telemetry_csv(
    ...
    try:
        ...
        # For ChromaDB index throttling during bulk upload
        ...
        for idx, raw_row in enumerate(csv_reader):
            ...
            # Select subset for vector store to avoid overloading ChromaDB
            if idx % vector_index_interval == 0:
                vector_batch.append(cleaned)
            
            if len(batch) >= BATCH_SIZE:
                # Bulk insert into DB
                db.bulk_insert_mappings(TelemetryRecord, batch)
                db.commit()
        ...
        # Optional: Index subset of telemetry records in ChromaDB in batches
        if vector_batch:
            try:
                for item in vector_batch:
                    mock_record = TelemetryRecord(**item)
                    index_telemetry_in_vector_db(mock_record)
